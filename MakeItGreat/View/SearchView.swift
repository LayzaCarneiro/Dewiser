...
struct SearchView: View {
    ...
    @State private var selectedDecision: CardModel?
    @State private var decisionsArray: [CardModel] = []

    var filteredDecisions: [CardModel] {
        if searchTerm.isEmpty {
            return decisionsArray
        } else {
            return decisionsArray.filter { $0.title.localizedCaseInsensitiveContains(searchTerm) }
        }
    }
                    ...
                    VStack {
                        List {
                            ...
                            .onMove { indices, newOffset in
                                decisionsArray.move(fromOffsets: indices, toOffset: newOffset)
                            }
                            .listRowBackground(Color.clear)
                            .listRowSeparator(.hidden)
                        }
                        ...
                        .background(Color.clear)
//                        .toolbar {
//                            EditButton() // Botão de editar para habilitar a movimentação
//                        }
                    }
                }
                .searchable(text: $searchTerm, placement: .navigationBarDrawer(
        ...
        .navigationBarTitleDisplayMode(.inline)
        .onAppear {
            decisionsArray = decisions // Atualiza o array de decisões
        }
        .onTapGesture {
           hideKeyboard()
        }
